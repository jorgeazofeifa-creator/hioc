...
- Pi-hole DHCP status reporting and device evidence now consume one cycle-local structured lease snapshot per inventory run, preventing duplicate file acquisition, duplicate parse warnings, and status/evidence disagreement while preserving standalone helper behavior and all existing parsing, assignment, identity, health, and public contracts. Production validation for this still-open checkpoint remains pending.
